...
def analyze_image_and_get_wager_results(uploaded_image):
    ...
    prompt = "Analyze the image and provide the results in a table with columns 'Amount Wagered' and 'Amount Won' and 'Date'. The image should only have one value for each column. You can typically find the information labeled with 'total wagered' and 'Return' something along those lines. DO NOT INCLUDE ANYTHING ELSE IN THE RESPONSE EXCEPT THE TABLE"
